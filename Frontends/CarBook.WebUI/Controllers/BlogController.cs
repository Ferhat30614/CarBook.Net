...
namespace CarBook.WebUI.Controllers
{
    public class BlogController : Controller
    {
        ...
        [HttpGet]
        public  IActionResult GetCommentsPartial(int blogId)
        {
            //var client = _httpClientFactory.CreateClient();
            //var responseMessage = await client.GetAsync("https://localhost:7192/api/Comments/GetCommentsByBlogIdWithReplies?id=" + blogId);
            
            //    var dataJson = await responseMessage.Content.ReadAsStringAsync();
            //    var values = JsonConvert.DeserializeObject<List<ResultCommentDto>>(dataJson);
                
                return ViewComponent("_CommentListByBlogComponentPartial", new { id = blogId });
